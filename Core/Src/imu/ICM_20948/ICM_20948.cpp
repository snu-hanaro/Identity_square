#include "imu/ICM_20948/ICM_20948.hpp"

#include "imu/ICM_20948/util/ICM_20948_REGISTERS.h"
#include "imu/ICM_20948/util/AK09916_REGISTERS.h"
...
void ICM_20948::enableDebugging(void)
{
...
{
  if (_printDebug == true)
    printf("%d", i);//_debugSerial->print(i);
}

void ICM_20948::debugPrintf(float f)
{
  if (_printDebug == true)
    printf("%f", f);//_debugSerial->print(f);
}

void ICM_20948::debugPrintStatus(ICM_20948_Status_e stat)
{
  debugPrint(statusString(stat));
}
...
ICM_20948_Status_e ICM_20948_I2C::begin(I2C_HandleTypeDef *i2cPort)
{
  // Associate
  _i2c = i2cPort;
...
ICM_20948_Status_e ICM_20948_SPI::begin(SPI_HandleTypeDef *spiPort, GPIO_TypeDef* gpioPort, uint16_t csPin, uint32_t SPIFreq)
{
  if (SPIFreq > 7000000)
...
ICM_20948_Status_e ICM_20948_write_I2C(uint8_t reg, uint8_t *data, uint32_t len, void *user)
{
  ...
  I2C_HandleTypeDef *_i2c = ((ICM_20948_I2C *)user)->_i2c; // Cast user field to ICM_20948_I2C type and extract the I2C interface pointer
  ...
  HAL_I2C_Mem_Write(_i2c, addr << 1, reg, 1, data, len, HAL_MAX_DELAY);
...
ICM_20948_Status_e ICM_20948_read_I2C(uint8_t reg, uint8_t *buff, uint32_t len, void *user)
{
  ...
  I2C_HandleTypeDef *_i2c = ((ICM_20948_I2C *)user)->_i2c;
  uint8_t addr = ((ICM_20948_I2C *)user)->_addr;
  if (_i2c == NULL)
  {
    return ICM_20948_Stat_ParamErr;
  ...
  if(HAL_I2C_Mem_Read(_i2c, addr << 1, reg, 1, buff, len, HAL_MAX_DELAY) != HAL_OK)
  {
...
ICM_20948_Status_e ICM_20948_write_SPI(uint8_t reg, uint8_t *data, uint32_t len, void *user)
{
  ...
  SPI_HandleTypeDef *_spi = ((ICM_20948_SPI *)user)->_spi;
  GPIO_TypeDef *_gpio = ((ICM_20948_SPI *)user)->_gpio;
  uint8_t cs = ((ICM_20948_SPI *)user)->_cs;
  if (_spi == NULL || _gpio == NULL)
  {
    return ICM_20948_Stat_ParamErr;
  }

  uint8_t regData = reg & 0x7F;

  HAL_GPIO_WritePin(_gpio, cs, GPIO_PIN_RESET);

  HAL_SPI_Transmit(_spi, &regData, 1, HAL_MAX_DELAY);

  HAL_GPIO_WritePin(_gpio, cs, GPIO_PIN_SET);
...
ICM_20948_Status_e ICM_20948_read_SPI(uint8_t reg, uint8_t *buff, uint32_t len, void *user)
{
  ...
  uint8_t data = ((reg & 0x7F) | 0x80);

  HAL_GPIO_WritePin(_gpio, cs, GPIO_PIN_RESET);

  HAL_SPI_Transmit(_spi, &data, 1, HAL_MAX_DELAY);
  HAL_SPI_Receive(_spi, buff, len, HAL_MAX_DELAY);
