...
static FLIGHT_STATE flight_state = STATE_CONF_DONE;

static float ground_ax, ground_ay, ground_az;
static float ground_a_abs;
...
FLIGHT_STATE init_flight_state_machine(const flight_state_machine_init_input &init_input){
   ...
   ground_a_abs = norm(ground_ax, ground_ay, ground_az);
   flight_state = STATE_CONF_DONE;
   return flight_state;
}
...
//   #endif
      ...
      && alt >= DROGUE_DEPLOY_MIN_ALT
         ...
         flight_state = STATE_DROGUE_DEPLOY;
